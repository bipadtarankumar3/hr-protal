    ...
    <div class="row g-3">
        <div class="col-md-6">
            ...
            <select name="employee_id" class="form-select @error('employee_id') is-invalid @enderror" required>
                ...
                @foreach($employees as $emp)
                    <option value="{{ $emp->id }}" {{ old('employee_id', $pulse_log->employee_id) == $emp->id ? 'selected' : '' }}>
                        {{ $emp->name }} ({{ $emp->email }})
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" 
                   value="{{ old('date', $pulse_log->date ? $pulse_log->date->format('Y-m-d') : '') }}" required>
        ...
        <div class="col-md-6">
            ...
            <input type="time" name="check_in_time" class="form-control @error('check_in_time') is-invalid @enderror" 
                   value="{{ old('check_in_time', $pulse_log->check_in_time ? date('H:i', strtotime($pulse_log->check_in_time)) : '') }}" required>
        ...
        <div class="col-md-6">
            ...
            <input type="time" name="check_out_time" class="form-control @error('check_out_time') is-invalid @enderror" 
                   value="{{ old('check_out_time', $pulse_log->check_out_time ? date('H:i', strtotime($pulse_log->check_out_time)) : '') }}">
        ...
        <div class="col-md-6">
            ...
            <input type="number" name="duration_hours" step="0.01" class="form-control @error('duration_hours') is-invalid @enderror" 
                   value="{{ old('duration_hours', $pulse_log->duration_hours) }}" min="0" max="24">
        ...
        <div class="col-md-6">
            ...
            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                <option value="present" {{ old('status', $pulse_log->status) == 'present' ? 'selected' : '' }}>Present</option>
                <option value="absent" {{ old('status', $pulse_log->status) == 'absent' ? 'selected' : '' }}>Absent</option>
                <option value="late" {{ old('status', $pulse_log->status) == 'late' ? 'selected' : '' }}>Late</option>
                <option value="on_leave" {{ old('status', $pulse_log->status) == 'on_leave' ? 'selected' : '' }}>On Leave</option>
            </select>
        ...
        <div class="col-md-12">
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" name="is_active" id="edit_is_active" value="1" 
                       {{ old('is_active', $pulse_log->is_active) ? 'checked' : '' }}>
